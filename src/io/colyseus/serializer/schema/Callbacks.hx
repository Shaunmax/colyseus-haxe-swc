...
class SchemaCallbacks<T> {
	...
	public function listen(instanceOrFieldName:Dynamic, fieldNameOrCallback:Dynamic, ?callbackOrImmediate:Dynamic, ?immediate:Bool = true) {
		var instance:IRef;
		...
		if (existing != null && Std.isOfType(existing, IRef) && existing.__refId != 0 && immediate == true && !this.isTriggering) {
	...
	public function onAdd(instanceOrFieldName:Dynamic, fieldNameOrCallback:Dynamic, ?callbackOrImmediate:Dynamic, ?immediate:Bool = true) {
		var instance:IRef;
		var fieldName:String;
		var callback:Dynamic->Dynamic->Void;
		if (Std.isOfType(instanceOrFieldName, String)) {
	...
	public function onChange(instanceOrFieldName:Dynamic, callbackOrProperty:Dynamic, ?callback:Dynamic->Dynamic->Void) {
		var instance:IRef;
		var fieldName:String;
		if (Std.isOfType(instanceOrFieldName, String)) {
			instance = cast this.decoder.state;
	...
	public function onRemove(instanceOrFieldName:Dynamic, callbackOrProperty:Dynamic, ?callback:Dynamic->Dynamic->Void) {
		var instance:IRef;
		var fieldName:String;
		if (Std.isOfType(instanceOrFieldName, String)) {
			instance = cast this.decoder.state;
	...
	public function addCallbackOrWaitCollectionAvailable(instance:IRef, fieldName:String, operation:OPERATION, callback:Dynamic->Dynamic->Void,
			?immediate:Bool = true) {
		var _this = this;
		var removeHandler = () -> {}
	...
	public function addCallback(refId:Int, operationOrFieldName:Dynamic, callback:Dynamic) {
		var key = (Std.isOfType(operationOrFieldName, String)) ? operationOrFieldName : "#" + operationOrFieldName;
	...
	public function triggerFieldCallbacks(callbacks:Map<String, Array<Dynamic>>, field:String, arg1:Dynamic, arg2:Dynamic) {
		...
		isTriggering = true;
		for (callback in callbacks[field]) {
			callback(arg1, arg2);
		}
		isTriggering = false;
	}
	...
	private function triggerChanges(allChanges:Array<DataChange>) {
		...
		for (change in allChanges) {
			...
			//
			if (((change.op & cast OPERATION.DELETE) == OPERATION.DELETE) && Std.isOfType(change.previousValue, Schema)) {
