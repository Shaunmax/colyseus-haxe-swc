...
public class TicTacToe extends Screen{
    ...
    private function onAddedToStage(event:Event):void {
        ...
        Starling.juggler.delayCall(joinServerRoom, 1);
        //joinServerRoom();
    }
            ...
            if(err){
                ...
                myroom.onMessage("data", function (message){
                    ...
                    if(message.current_turn == myroom.sessionId){
                        current_player = _you;
                        card.touchable = true;
                        Starling.juggler.tween(backGround_color, 0.5, { "color": 0xFF5555 });
                    }else{
                        current_player = _opponent;
                        card.touchable = false;
                        Starling.juggler.tween(backGround_color, 0.5, { "color": 0x5589FF });
                    }
                ...
                myroom.onMessage("result", function (message){
                    ...
                    if(game_result.winner == myroom.sessionId){
                        if(Number(game_result.status) == 1){
                            Game.showToast("OPPONENT LEFT - YOU WIN!",Color.GREEN,Color.WHITE,Number.POSITIVE_INFINITY);
                        }else {
    ...
    public static function displayResult():void{
        if(game_result.winner == myroom.sessionId){
            if(Number(game_result.status) == 0){
                Game.showToast("GAME OVER - YOU WON!",Color.GREEN,Color.WHITE,Number.POSITIVE_INFINITY);
            }else {
                Game.showToast("OPPONENT LEFT - YOU WIN!",Color.GREEN,Color.WHITE,Number.POSITIVE_INFINITY);
            }

        }else{
            Game.showToast("GAME OVER - YOU LOST!",Color.RED,Color.WHITE,Number.POSITIVE_INFINITY);
        }
    }

    private function onGameStateChange(message):void{
        trace("data received - " + JSON.stringify(message.state));

        switch (Number(message.state)) {
            case 0:
                Game.showToast("WAITING FOR OPPONENT",Color.BLUE,Color.WHITE,Number.POSITIVE_INFINITY);
                break;

            case 1:
                Game.showToast("MATCH WILL BEGIN SHORTLY",Color.FUCHSIA,Color.WHITE);
            ...
            case 2:
                stopTween();
                Game.showToast("DRAW MATCH",Color.PURPLE,Color.WHITE,Number.POSITIVE_INFINITY);
                break;
            case 3:
                trace("Room Closed!");
            ...
            default:
                Game.showToast("OOPS! SOMETHING WENT WRONG",Color.BLACK,Color.WHITE,Number.POSITIVE_INFINITY);
        }
    }
